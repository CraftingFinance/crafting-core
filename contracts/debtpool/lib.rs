...
mod debtpool {
    ...
    impl SDP {
        #[ink(constructor)]
        pub fn new(aggregator_account: AccountId, param_account: AccountId) -> Self {
            let exchange_prices: ExchangePrices = FromAccountId::from_account_id(aggregator_account);
            let system_param: Param = FromAccountId::from_account_id(param_account);
            ...
            Self {
                ...
                system_param: Lazy::new(system_param),
        ...
        pub fn get_asset_amount(&self, asset: AccountId) -> Balance {
            assert!(self.system_param.is_effective_synthetic_asset(asset));
        ...
        pub fn get_user_asset_total_value(&self, user: AccountId) -> u128 {
            ...
            self.query_user_asset_total_value(user)
        }

        fn query_user_asset_total_value(&self, user: AccountId) -> u128 {
            let mut total: u128 = 0;
        ...
        pub fn get_user_profit(&self, user: AccountId) -> i128 {
            ...
            self.query_user_asset_total_value(user) as i128 -
                (self.get_asset_total_value() * self.query_user_debt_ratio(user)) as i128 / 10000
        }
        ...
        pub fn get_user_debt_ratio(&self, user: AccountId) -> u128 {
            ...
            self.query_user_debt_ratio(user)
        }

        fn query_user_debt_ratio(&self, user: AccountId) -> u128 {
        ...
        pub fn join(&mut self, user: AccountId, collateral_asset: AccountId, collateral_amount:Balance,
            ...
            assert!(self.system_param.is_effective_collateral_asset(collateral_asset));
            assert!(self.system_param.is_effective_synthetic_asset(synthetic_asset));
            assert!(collateral_amount > 0);
            assert!(synthetic_amount > 0);
            ink_env::debug_println!("debtpool 111");
            ...
            ink_env::debug_println!("debtpool leverage_ratio: {}", leverage_ratio);
            let (min, max) = self.system_param.get_leverage_ratio();
        ...
        pub fn swap_in_sdp(&mut self, user: AccountId, old_asset: AccountId, new_asset: AccountId,
                    swap_amount: Balance) {
            assert_ne!(user, Default::default());
            assert!(self.system_param.is_effective_synthetic_asset(old_asset));
            assert!(self.system_param.is_effective_synthetic_asset(new_asset));
        ...
        fn calc_debt_ratio(&mut self, old_total_value: u128, new_total_value: u128) {
            ...
            for (_, debt_ratio) in self.user_debt_ratios.iter_mut() {
                *debt_ratio = (old_total_value * (*debt_ratio)) / new_total_value;
                ink_env::debug_println!("calc_debt_ratio old_total_value: {}, new_total_value: {}, debt_ratio: {}",
                                       old_total_value, new_total_value, debt_ratio);
            }
        }
        ...
        fn is_query_authority(&self, user: AccountId) {
            assert_ne!(user, Default::default());
